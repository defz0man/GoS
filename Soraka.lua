	...
	if Ready(_W) or Ready(_R) then
		for _,champ in pairs(GetAllyHeroes()) do
			...
			if Config.h.AR:Value() then
				if not IsDead(champ) and (GetCurrentHP(champ)/GetMaxHP(champ)<(healvalue*0.5)) and EnemiesAround(GetOrigin(champ), 750)>0 then
					CastSpell(_R)
				end
			end
		end
...
function combo(unit)
	if IOW:Mode() == "Combo" then
		if Config.c.Q:Value() and Ready(_Q) and ValidTarget(unit,GetCastRange(myHero,_Q)*0.85) then
		...
		if Config.c.E:Value() and Ready(_E) and ValidTarget(unit,GetCastRange(myHero,_E)) then
			local EnemyOrg=GetOrigin(unit)
			CastSkillShot(_E,EnemyOrg.x,EnemyOrg.y,EnemyOrg.z)
		end
	end
