...
function combo(unit)
	...
	if IOW:Mode() == "Combo" then
		--Q1
		if VMenu.c.Q:Value() and Ready(_Q) and ValidTarget(unit, 1500) and not qCharge then
		...
		--E
		if Ready(_E) and VMenu.c.E:Value() and ValidTarget(unit, GetCastRange(myHero,_E)) then
...
function StackDamage(unit)
	if wTrack[unit] then
		local wDmg = GetMaxHP(unit)*((1.25+GetCastLevel(myHero,_W)*0.75)+GetBonusAP(unit)*0.02)*.01*.95
